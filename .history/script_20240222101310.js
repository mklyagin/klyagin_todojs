...
const saveEdit = (taskTitle, editTaskText) => {
  ...
  editVisibilityToggle(editTaskText);
};
...
const eventUlHandlerListener = (event) => {
  ...
  switch (event.target.className) {
    ...
    case ('edit-task'):
      ...
      event.target.addEventListener('blur', saveEdit(event.target.previousElementSibling, event.target));
      break;
    case ('task-title'):
      editVisibilityToggle(event.target);
      event.target.previousElementSibling.addEventListener('blur', saveEdit(event.target.previousElementSibling, event.target));
      event.target.previousElementSibling.addEventListener('keydown', saveEdit(event.target.previousElementSibling, event.target));
      break;
    default:
