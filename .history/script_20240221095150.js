...
const eventUlHandlerListener = (event) => {
  ...
  switch (event.target.type) {
    case 'checkbox':
      ...
      checkedTask = taskList.filter((task) => Number(task.id) === Number(event.target.parentNode.id));
      checkedTask.isDone = event.target.checked;
      console.log(checkedTask, '   ', checkedTask.isDone);
      break;
    case ('button'):
