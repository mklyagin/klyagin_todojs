...
const tabulationListener = (event) => {
  ...
  switch (event.target.id) {
    case ('show-all'):
      filterType = 'all';
      currentPage = Math.abs(Math.ceil((tabNumbers.total - 1) / 5));
      console.log('Form tab', currentPage);
      taskRender();
      break;
    case ('show-active'):
      filterType = 'active';
      currentPage = Math.abs(Math.ceil((tabNumbers.active - 1) / 5));
      console.log('Form tab', currentPage);
      taskRender();
      break;
    case ('show-completed'):
      filterType = 'completed';
      currentPage = Math.abs(Math.ceil((tabNumbers.completed - 1) / 5));
      console.log('Form tab', currentPage);
      taskRender();
      break;
    default:
