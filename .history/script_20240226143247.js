...
const listToPages = (currentList) => {
  ...
  currentList.forEach((task) => {
    if (taskListPages[index].length === QUANITY_OF_TASKS) {
      taskListPages.push([]);
      index += 1;
    }
...
const setPage = (event) => {
  if (event.target.class === 'page-number') {
...
const taskFilter = (currentList) => {
  switch (filterType) {
    case ('all'):
      return taskListPages[currentPage];
    case ('active'):
      currentList = taskListPages[currentPage].filter((task) => !task.isDone);
      return currentList;
    case ('completed'):
      currentList = taskListPages[currentPage].filter((task) => task.isDone);
      return currentList;
...
const taskRender = () => {
  ...
  let currentList = [];
  taskValidation();
  listToPages(taskList);
  currentList = taskFilter(currentList);

  currentList[currentPage].forEach((task) => {
    listOfTasks
    += `<li id=${task.id}>
